...
#Created 2022-02-23 at 08:07

rplots.level <- 1
xpose.runno <- '15'
toolname <- 'execute'
pdf.filename <- paste0('PsN_',toolname,'_plots.pdf')
pdf.title <- 'execute diagnostic plots run 15'
working.directory<-'C:\\Projects\\Abcuro\\ABC008 FiM PKPD\\Main Development\\WorkArea\\NONMEM\\run15\\'
model.directory<-'C:\\Projects\\Abcuro\\ABC008 FiM PKPD\\Main Development\\WorkArea\\NONMEM\\'
results.directory <- 'C:\\Projects\\Abcuro\\ABC008 FiM PKPD\\Main Development\\WorkArea\\NONMEM\\'
model.filename<-'run15.mod'
subset.variable<-NULL
mod.suffix <- '.mod'
mod.prefix <- 'run'
tab.suffix <- ''
rscripts.directory <- 'C:/Perl64/site/lib/PsN_4_8_1/R-scripts'
raw.results.file <- 'raw_results_run15.csv'
theta.labels <- c('PD0','THALF KOUT','EMAX','EC50','KTR','Additive error (log)','Proportional error')
theta.fixed <- c(FALSE,FALSE,FALSE,TRUE,FALSE,TRUE,FALSE)
omega.labels <- c('IIV(PD0)','IIV(KOUT)','IIV(EMAX)','IIV(EC50)','IIV(KTR)')
omega.fixed <- c(FALSE,FALSE,FALSE,TRUE,TRUE)
sigma.labels <- c('SIGMA')
sigma.fixed <- c(TRUE)
n.eta <- 5
n.eps <- 1
